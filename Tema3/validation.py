...
def checkString(file,str):
    ...
    for letter in str:
        ...
        for state in listStates:
            for transition in transitions:
                if letter in transition[1]:       
                    if listStates[index] == transition[0]:    
                        if transition[0] == listStates:
                            listStates[index] = transition[2]
                        else:
                            listStates.append(transition[2])
                            index+=1

    print(listStates)
    listStates = set(listStates)            

    if any(x in listStates for x in F_State):
        print('accept')
        print('final states:',len(listStates))
    else:
        print('reject')
